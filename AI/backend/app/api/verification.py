...
async def calculate_credit(request: CreditScoreRequest):
    ...
    job_bonus = min(request.completed_jobs * 2, 20)          # 完成兼职加分，上限+20
    review_penalty = request.bad_reviews * 5                  # 每条差评扣5分
    violation_penalty = request.violation_count * 10          # 每次违规扣10分
    rating_penalty = max(0, (4.5 - request.avg_rating) * 10)  # 低于4.5分时扣分
