...
exports.createJob = async (req, res) => {
  try {
    ...
    if (isNaN(numericSalary) || numericSalary < 0 || numericSalary > 10000000) {
      return res.status(400).json({
        success: false,
    ...
    // salaryType 白名单校验
    const validSalaryTypes = ['hourly', 'daily', 'weekly', 'monthly'];
    ...
    // XSS 过滤
    ({ title, description, requirements, location, workingHours } = sanitizeFields(
    ...
    // 异步调用 AI 虚假岗位检测
    const companyName = verification ? verification.companyName : '';
    ...
    checkFraud({
      ...
      .then(fraudResult => {
        if (fraudResult) {
          job.update({ fraudCheckResult: fraudResult });
        }
      })
      .catch(err => console.error('AI虚假检测失败:', err));
...
exports.getJobById = async (req, res) => {
  try {
    ...
    // 未审核/已拒绝岗位仅允许管理员或发布者查看
    if (job.auditStatus !== 'approved') {
      const isAdmin = req.user && req.user.role === 'admin';
      const isOwner = req.user && req.user.id === job.employerId;
      if (!isAdmin && !isOwner) {
        return res.status(403).json({
          success: false,
          message: '该岗位尚未公开'
...
exports.updateJob = async (req, res) => {
  try {
    ...
    if (req.user.role === 'admin' && status !== undefined) {
      ...
      if (!validStatusTransitions[job.status] || !validStatusTransitions[job.status].includes(status)) {
        return res.status(400).json({
          success: false,
          message: '不允许的岗位状态转换'
        });
...
exports.deleteJob = async (req, res) => {
  try {
    ...
    await require('../config/database').sequelize.transaction(async (t) => {
      ...
      if (!job) {
        throw { status: 404, message: '岗位不存在' };
      }

      // 检查是否为发布该岗位的企业（admin 可删除所有岗位）
      if (req.user.role !== 'admin' && job.employerId !== req.user.id) {
        throw { status: 403, message: '您没有权限删除此岗位' };
      }

      // 级联删除关联的申请和收藏
      const { Application, Bookmark } = require('../models');
      await Application.destroy({ where: { jobId: id }, transaction: t });
      await Bookmark.destroy({ where: { jobId: id }, transaction: t });
...
exports.rejectJob = async (req, res) => {
  try {
    ...
    const cleanReason = require('../utils/sanitize').sanitizeText(reason || '不符合发布要求');
